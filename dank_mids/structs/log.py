...
class SmallLog(TinyLog, frozen=True, kw_only=True):  # type: ignore [call-arg]
    ...
    def address(self) -> Optional[Address]:
        return msgspec.json.decode(self._address, type=Optional[Address], dec_hook=Address._decode_hook)
    @cached_property
...
class Log(SmallLog, frozen=True, kw_only=True):  # type: ignore [call-arg]
    ...
    def blockNumber(self) -> Optional[uint]:
        return msgspec.json.decode(self._blockNumber, type=Optional[uint], dec_hook=uint._decode_hook)
    @cached_property
    def transactionHash(self) -> HexBytes:
        return msgspec.json.decode(self._transactionHash, type=HexBytes, dec_hook=_decode_hexbytes)
    _transactionIndex: msgspec.Raw = msgspec.field(name="transactionIndex")
    @cached_property
    def transactionIndex(self) -> Optional[uint]:
        return msgspec.json.decode(self._transactionIndex, type=Optional[uint], dec_hook=uint._decode_hook)
    @cached_property
    def logIndex(self) -> Optional[uint]:
        return msgspec.json.decode(self._logIndex, type=Optional[uint], dec_hook=uint._decode_hook)
    @property
...
class FullLog(Log, frozen=True, kw_only=True, forbid_unknown_fields=True):  # type: ignore [call-arg]
    ...
    def blockHash(self) -> Optional[HexBytes]:
        return msgspec.json.decode(self._blockHash, type=Optional[HexBytes], dec_hook=_decode_hexbytes)
