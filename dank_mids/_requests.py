...
class _Batch(_RequestBase[list[_Response]], Iterable[_Request]):
    ...
    def __init__(
        self,
        controller: "DankMiddlewareController",
        calls: Iterable[_Request],
        uid: BatchId | None = None,
    ) -> None:
        super().__init__(controller, uid, DebuggableFuture(self, controller._loop))
        self.calls = WeakList(calls)
        self._done = _RequestEvent(self)

    def __len__(self) -> int:
        return get_len_for_method(self.method) if isinstance(self, Multicall) else len(self.calls)

    def __repr__(self) -> str:
        batch = self._batch
        batch_info = "" if batch is None else f" batch={batch}"
        return f"<{self.__class__.__name__} uid={self.uid} len={len(self)}{batch_info} awaited={self._awaited}>"
...
class Multicall(_Batch[RPCResponse, eth_call]):
    __slots__ = ("bid", "_handler", "_coro", "_callbacks")
    _awaited: bool = False
    method: Final = "eth_call"
    params: tuple[MulticallChunk, BlockId]
    _missing: Final[DebuggableFuture[set[eth_call] | list[eth_call]]]
    ...
    def __init__(
        ...
        """
        Initialize a new Multicall.

        Args:
            controller: The DankMiddlewareController that manages this multicall.
            calls: A list of eth_call objects that will be included in the multicall.
            uid: A unique identifier for this multicall. If none is provided, one will be created.
        """
        super().__init__(controller, calls, uid)
        self.bid = uid or self.uid
        self._coro = self._call()
        self._handler = _RequestEvent(self)
        self._callbacks: set[Future] = set()
        self._missing = DebuggableFuture(self, controller._loop)
    ...
    def __bool__(self) -> bool:
        return bool(self._coro)

    def __await__(self) -> Generator[Any, None, RPCResponse]:
        return self.get_response().__await__()

    def __del__(self) -> None:
        """Log an error if this multicall is garbage collected before completion."""
        try:
            if self and not self._done.is_set():
            ...
            # NOTE: we can't log the error if __del__ is raising because of a garbage collection issue.
            return
    ...
    def method_counts(self) -> dict[RPCEndpoint, int]:
        """
        Count the occurrences of each method in the multicall.
    ...
    @property
    def batch(self) -> Optional["JSONRPCBatch"]:
        return self._batch

    @batch.setter
    def batch(self, batch: Optional["JSONRPCBatch"]) -> None:
        if batch is not None:
            self._batch = batch

    @property
    def block(self) -> BlockId:
        # NOTE: Its fine to return the BlockId as we stored it, not its decoded int value.
    ...
    @property
    def multicall_contract(self) -> MulticallContract | None:
        return self.controller.multicall

    @property
    def clearable(self) -> bool:
        return self._handler.is_set()

    @property
    def ignored(self) -> bool:
        # if this is set, we have to rebuild the multicall (with new data)
        return self._missing.done() and len(self._missing.result()) > 0

    @property
    def request_history(self) -> list[Task]:
        return self._handler.request_history

    @property
    def update_key(self) -> tuple:
        return self.method, self.params[1], len(self)

    async def _call(self) -> RawResponse:
        self.params = (mcall_encode(self.calls), self.block)
        for i, call in enumerate(self.calls):
            call._batch = self
            call._batch_uid = f"{self.uid}-{i}"
            call._batch_index = i
        response: RawResponse = await self.controller.make_request(
            self.method,
            self.params,
            request_id=self.uid,
        )
    ...
    async def post(self) -> tuple[list[RawResponse], list[Multicall | RPCRequest]]:
        ...
        try:
            ...
            if ENVS.DEBUG:  # type: ignore [attr-defined]
                self._record_failure(e, data.decode())
            raise
        except Exception as e:
