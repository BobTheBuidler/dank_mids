    ...
    async def get_response(self) -> None:
        ...
        try:
            ...
            if e.message == "Payload Too Large":
                ...
                await _debugging.failures.record(self.controller.chain_id, e, self.calldata)
            else:
                if _log_exception(e):
                    await _debugging.failures.record(self.controller.chain_id, e, self.calldata)
...
class JSONRPCBatch(_Batch[Union[Multicall, RPCRequest]]):
    ...
    async def get_response(self) -> None:
        ...
        # I want to see these asap when working on the lib.
            ...
            if _log_exception(e):
                await _debugging.failures.record(self.controller.chain_id, e, self.data)
    ...
    async def post(self) -> List[RawResponse]:
        ...
        try:
            ...
            if ENVS.DEBUG:
                _debugging.failures.record(self.controller.chain_id, e, self.data)
            raise e
        except Exception as e:
        ...
        if ENVS.DEBUG:
            _debugging.failures.record(self.controller.chain_id, response.exception, self.data)
        raise response.exception
