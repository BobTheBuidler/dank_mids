    ...
    async def get_response(self) -> None:
        ...
        try:
            ...
            if e.message == "Payload Too Large":
                ...
                _debugging.failures.record(self.controller.chain_id, e, type(self).__name__, self.calldata)
            else:
                if _log_exception(e):
                    _debugging.failures.record(self.controller.chain_id, e, type(self).__name__, self.calldata)
...
class JSONRPCBatch(_Batch[Union[Multicall, RPCRequest]]):
    ...
    async def get_response(self) -> None:
        ...
        # I want to see these asap when working on the lib.
            ...
            if _log_exception(e):
                _debugging.failures.record(self.controller.chain_id, e, type(self).__name__, self.data)
    ...
    async def post(self) -> List[RawResponse]:
        ...
        if ENVS.DEBUG:
            _debugging.failures.record(self.controller.chain_id, response.exception, type(self).__name__, self.data)
        raise response.exception
