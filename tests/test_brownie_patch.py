...
async def test_patch_contract_call():
    ...
    assert await weth.totalSupply.coroutine(block_identifier=13_000_000, decimals=18) == Decimal(
        "6620041.514474872981393155"
    )
...
async def test_patch_contract_tx():
    ...
    uni_v3_quoter = dank_mids.patch_contract(
        get_contract("0xb27308f9F90D607463bb33eA1BeBb41C27CE5AB6")
    )
    assert hasattr(uni_v3_quoter.quoteExactInput, "coroutine")
    assert (
        await uni_v3_quoter.quoteExactInput.coroutine(
            b"\xc0*\xaa9\xb2#\xfe\x8d\n\x0e\\O'\xea\xd9\x08<ul\xc2\x00\x01\xf4\xa0\xb8i\x91\xc6!\x8b6\xc1\xd1\x9dJ.\x9e\xb0\xce6\x06\xebH",
            1e18,
            block_identifier=13_000_000,
        )
        == 3169438072
    )
    assert await uni_v3_quoter.quoteExactInput.coroutine(
        b"\xc0*\xaa9\xb2#\xfe\x8d\n\x0e\\O'\xea\xd9\x08<ul\xc2\x00\x01\xf4\xa0\xb8i\x91\xc6!\x8b6\xc1\xd1\x9dJ.\x9e\xb0\xce6\x06\xebH",
        1e18,
        block_identifier=13_000_000,
        decimals=8,
...
async def test_dank_contract_call():
    ...
    assert (
        await dank_weth.totalSupply.coroutine(block_identifier=13_000_000)
        == 6620041514474872981393155
    )
    assert await dank_weth.totalSupply.coroutine(
        block_identifier=13_000_000, decimals=18
    ) == Decimal("6620041.514474872981393155")
...
async def test_dank_contract_tx():
    ...
    assert (
        await uni_v3_quoter.quoteExactInput.coroutine(*args, block_identifier=13_000_000)
        == 3169438072
    )
    assert await uni_v3_quoter.quoteExactInput.coroutine(
        *args, block_identifier=13_000_000, decimals=8
    ) == Decimal("31.69438072")
...
async def test_dank_overloaded_method():
    ...
    assert (
        call(pool)
        == curve_factory.get_lp_token(pool)
        == await curve_factory.get_lp_token.coroutine(pool)
        == "0x9432242D9Dc10AD5e2Fe570b8FC053A16597Caf6"
    )
...
_CURVE_FACTORY_ABI = [
    ...
    },
    {
        ...
        "type": "function",
        "name": "get_gauge",
        "inputs": [{"name": "_pool", "type": "address"}, {"name": "gauge_idx", "type": "uint256"}],
        "outputs": [{"name": "", "type": "address"}],
    },
    {
        "stateMutability": "view",
