...
1) Fully test your changes before submitting a PR (run the full suite or all relevant tests): `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest <files>`
...
6) All new or edited code must have 90%+ coverage in tests/unit full suite.
...
9) If the repo uses mypyc, verify tests run against compiled extensions (not interpreted Python) and note how you confirmed.
...
13) Centralize pytest settings (flags, markers, ignore patterns, and targets) in pyproject.toml, pytest.ini, setup.cfg, or tox.ini; workflows/hooks should call pytest without inline args.
